...
bool FMjStepServerNoManagerGuard::RunTest(const FString& Parameters)
{
    ...
    if (!Disp) { AddError(TEXT("Server has no dispatcher")); Server->RemoveFromRoot(); return false; }
    ...
    auto MakeReq = [](const FString& Op) -> TSharedPtr<FJsonObject>
    {
        ...
        if (Op == TEXT("set_paused"))     R->SetBoolField  (TEXT("paused"), true);
        if (Op == TEXT("set_sim_speed"))  R->SetNumberField(TEXT("percent"), 100.0);
    ...
    // invalidate the planted "test-session" used by AssertNoManager.
    {
        ...
        FString OpField; bool bMgr = true;
...
{
    UURLabBridgeServer* Server = NewObject<UURLabBridgeServer>();
    if (!Server) { AddError(TEXT("NewObject<UURLabBridgeServer> failed")); return false; }
    Server->AddToRoot();
...
bool FMjStepServerPauseFlag::RunTest(const FString& Parameters)
{
    ...
    if (!Disp) { AddError(TEXT("Manager has no StepDispatcher")); S.Cleanup(); return false; }
...
bool FMjStepServerPuppetHandler::RunTest(const FString& Parameters)
{
    ...
    if (!m || !d) { AddError(TEXT("Model/data missing")); S.Cleanup(); return false; }

    int OnPostStepCount = 0;
    S.Manager->PhysicsEngine->OnPostStep = [&](mjModel*, mjData*) { OnPostStepCount++; };
    ...
    if (m->nq > 0) Req.QPos[0] = 0.42;
    if (m->nv > 0) Req.QVel[0] = 0.13;
    Req.Time = 1.5;
    ...
    if (m->nq > 0) TestEqual(TEXT("qpos[0] written"), (double)d->qpos[0], 0.42, 1e-9);
    if (m->nv > 0) TestEqual(TEXT("qvel[0] written"), (double)d->qvel[0], 0.13, 1e-9);
...
bool FMjStepServerDirectCtrl::RunTest(const FString& Parameters)
{
    ...
    if (Local.StartsWith(Pfx)) Local = Local.Mid(Pfx.Len());
...
bool FMjStepServerMsgpackRoundtrip::RunTest(const FString& Parameters)
{
    ...
    if (!bOk || !Round.IsValid()) { return false; }

    FString S; double D = 0; bool B = false;
    TestTrue(TEXT("op survives"),    Round->TryGetStringField(TEXT("op"), S));   TestEqual(TEXT("op value"),   S, FString(TEXT("step_ok")));
    TestTrue(TEXT("time survives"),  Round->TryGetNumberField(TEXT("time"), D)); TestEqual(TEXT("time value"), D, 1.234, 1e-9);
    TestTrue(TEXT("ok survives"),    Round->TryGetBoolField  (TEXT("ok"), B));   TestTrue(TEXT("ok value"),    B);
...
bool FMjStepServerXfrcApplied::RunTest(const FString& Parameters)
{
    ...
    mjModel* m = S.Manager->PhysicsEngine->GetModel();
    mjData*  d = S.Manager->PhysicsEngine->GetData();
    if (!m || !d || m->nbody < 2) { AddInfo(TEXT("Skipping xfrc test: <2 bodies")); S.Cleanup(); return true; }

    // Find the first non-world body's name.
    TArray<AMjArticulation*> Arts = S.Manager->GetAllArticulations();
    if (Arts.Num() == 0 || !Arts[0]) { S.Cleanup(); return true; }
    AMjArticulation* Art = Arts[0];

    TArray<UMjBody*> Bodies;
    Art->GetComponents<UMjBody>(Bodies);
    if (Bodies.Num() == 0) { AddInfo(TEXT("Skipping xfrc test: articulation has no bodies")); S.Cleanup(); return true; }

    UMjBody* B = nullptr;
    for (UMjBody* Bd : Bodies) { if (Bd && !Bd->bIsDefault) { B = Bd; break; } }
    if (!B) { S.Cleanup(); return true; }

    int32 BodyId = B->GetMjID();
    if (BodyId <= 0 || BodyId >= m->nbody) { S.Cleanup(); return true; }
    ...
    if (!BodyNameC) { AddInfo(TEXT("Skipping xfrc test: compiled body has no name")); S.Cleanup(); return true; }
...
bool FMjStepServerPerturbationSnapshot::RunTest(const FString& Parameters)
{
    FMjUESession S;
    if (!S.Init()) { AddError(S.LastError); return false; }
    if (!S.Manager->Perturbation) { AddError(TEXT("Manager has no Perturbation")); S.Cleanup(); return false; }
...
bool FMjStepServerSetQposActorId::RunTest(const FString& Parameters)
{
    ...
    TArray<TSharedPtr<FJsonValue>> Q; Q.Add(MakeShared<FJsonValueNumber>(0.91));
    Req->SetArrayField(TEXT("qpos"), Q);
...
bool FMjStepServerSetQposFreeBase::RunTest(const FString& Parameters)
{
    ...
    for (int i = 0; i < 7; ++i) Q.Add(MakeShared<FJsonValueNumber>(Vals[i]));
    Req->SetArrayField(TEXT("qpos"), Q);
...
bool FMjStepServerDispatchMutexNarrow::RunTest(const FString& Parameters)
{
    ...
    Server->Start(TEXT(""));
    FURLabRpcDispatcher* Disp = Server->GetDispatcher();
    if (!Disp) { AddError(TEXT("dispatcher missing")); Server->RemoveFromRoot(); return false; }
...
bool FMjStepServerStopDrains::RunTest(const FString& Parameters)
{
    ...
    URLabOpRegistry::RegisterHandler(TEXT("phase04_drainable_op"),
        [Disp, &bHandlerStarted, &bHandlerExitedDueToDrain]
...
bool FMjStepServerShmRejectsEditorOps::RunTest(const FString& Parameters)
{
    ...
    if (!Reply.IsValid()) { AddError(TEXT("reply did not decode")); Server->RemoveFromRoot(); return false; }
    ...
    FString ZmqCode;
    if (ZmqReply.IsValid()) ZmqReply->TryGetStringField(TEXT("code"), ZmqCode);
    ...
    for (const TSharedPtr<FJsonValue>& V : *Ops)
    {
        ...
        if ((*O)->TryGetArrayField(TEXT("reply_fields"), Fields) && Fields)
        {
            ...
            for (const TSharedPtr<FJsonValue>& F : *Fields)
            {
                ...
                if (S == TEXT("time:float")) bSawTime = true;
                if (S == TEXT("sim_time:object?")) bSawSimTime = true;
            }
